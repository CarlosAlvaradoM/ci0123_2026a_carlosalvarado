...
void* Cliente::ejecutar(void* arg){
    ...
    cout << "Comando -----> CONNECT " << id << endl;
    cout << "Comando -----> List " << req.indice << endl;
    ...
    // Avisar que este cliente ya envio su solicitud
    pthread_mutex_lock(&IPC::mutex_contador);
    IPC::clientes_listos++;

    if(IPC::clientes_listos == NUM_CLIENTES){
        // Desbloquear interceptor una vez
        sem_post(&IPC::sem_todos_clientes);
    }
