...
int VSocket::TryToConnect( const char * hostip, int port ) {
   ...
   if ( this->IPv6 ) {
      ...
      if ( connect(this->sockId, (struct sockaddr*)&addr6, sizeof(addr6)) < 0 ) {
         throw std::runtime_error("IPv6 connect()");
      }
      ...
      if ( connect(this->sockId, (struct sockaddr*)&addr, sizeof(addr)) < 0 ) {
         throw std::runtime_error("IPv4 connect()");
      }
   }
...
int VSocket::Bind( int port ) {
   ...
   if ( this->IPv6 ) {
      ...
      if ( bind(this->sockId, (struct sockaddr*)&addr6, sizeof(addr6)) < 0 ) {
      ...
      if ( bind(this->sockId, (struct sockaddr*)&addr, sizeof(addr)) < 0 ) {
         throw std::runtime_error("VSocket::Bind IPv4()");
      }
   }
