...
SSLSocket::SSLSocket( bool IPv6 ) {
   this->Init( 's', IPv6 );  // Pasa el flag IPv6 a Init

   this->Context = nullptr;
   this->BIO = nullptr;

   this->InitSSL();  // Inicializa como cliente
}
...
void SSLSocket::InitSSL( bool serverContext ) {
   ...
   if ( nullptr == this->Context ) {
      throw std::runtime_error( "SSLSocket::InitSSL, context is null" );
   }
...
void SSLSocket::InitContext( bool serverContext ) {
   ...
   // Selecciona el metodo TLS segun el rol
   if ( serverContext ) {
 ...
 **/
 void SSLSocket::LoadCertificates( const char * certFileName, const char * keyFileName ) {
   ...
   if ( SSL_CTX_use_certificate_file( ctx, certFileName, SSL_FILETYPE_PEM ) <= 0 ) {
      throw std::runtime_error( "SSLSocket::LoadCertificates, can't load certificate" );
   }
   
   // Load private key
   if ( SSL_CTX_use_PrivateKey_file( ctx, keyFileName, SSL_FILETYPE_PEM ) <= 0 ) {
      throw std::runtime_error( "SSLSocket::LoadCertificates, can't load private key" );
   }
   
   // Verify private key
   if ( !SSL_CTX_check_private_key( ctx ) ) {
      throw std::runtime_error( "SSLSocket::LoadCertificates, private key doesn't match certificate" );
   }
}
...
size_t SSLSocket::Write( const char * string ) {
   ...
   st = SSL_write( ssl, string, strlen(string) );
